...
all_items=[]
for f in glob.iglob("data\\img\\items\\*.png"):
	all_items.append(f.replace('data\\img\\items\\','').replace('.png',''))
def receipe(itm):
	name=str(itm)
	...
	if type_.lower() in ['c','f']:
		if type_.lower().startswith('c'):
			...
			all_['reward']={'id':name,'count':int(input('Reward count?'))}
	with open(f'data\\receipes\\{name}.json','w') as f:
		f.write(json.dumps(all_,indent=4))
